...
for reg in regions:
    ...
    for elem in pod_regions:
        ...
        for i in href:
            ...
            geo_url = "https://tools.paintmaps.com/" + geojson
            geo_response = requests.get(geo_url)
            if name_geo == "Nemeckij Nacional'nyj.geojson":
                name_geo = "Nemeckij_Nacional'nyj.geojson"
            if name_geo == "Uglegorsk (ZATO).geojson":
                name_geo = "Uglegorsk_(ZATO).geojson"
            if name_geo == "ZATO Mezhgor'e.geojson":
                name_geo = "ZATO_Mezhgor'e.geojson"
            if name_geo == "Dagestanskie Ogni.geojson":
                name_geo = "Dagestanskie_Ogni.geojson"
            if name_geo == "Franz Josef Land.geojson":
                name_geo = "Franz_Josef_Land.geojson"
            if name_geo == "Novaja Zemlja.geojson":
                name_geo = "Novaja_Zemlja.geojson"
            if name_geo in regions_cnt:
                regions_cnt[name_geo] = regions_cnt[name_geo] + 1
                name_geo = name_geo[:-8] + str(regions_cnt[name_geo]) + ".geojson"
            else:
